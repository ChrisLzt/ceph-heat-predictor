...
TRACE_RECORD_SIZE = 184
TRACE_MAX_FEATURES = 8
FEATURE_NAMES = (
    "past_access_count_log_margin",
    "previous_access_interval_encoded",
    "current_heat_log2p1",
    "projected_count_margin",
    "short_window_access_count_log2p1",
)
...
class WorkloadAggregate:
    ...
    def add(
        ...
        label_margin = math.log2(1.0 + future_access_count) - math.log2(
            1.0 + future_access_threshold
        )
        self.margin_counts[
            (
...
class TraceAnalyzer:
    ...
    def consume(self, row: Mapping[str, str], context: RecordContext) -> None:
        ...
        prediction_context_threshold = _integer(
            row, "future_access_threshold_at_prediction")
        future_window_access_threshold = _integer(
            row, "future_window_access_threshold")
        past_window_access_count = _integer(
            row, "past_window_access_count")
        future_access_count = _integer(row, "future_window_access_count")
        ...
        if prediction_context_threshold <= 0:
            raise ValueError(
                "future_access_threshold_at_prediction must be positive")
        if future_window_access_threshold <= 0:
            raise ValueError(
                "future_window_access_threshold must be positive")
        ...
        expected_actual = int(
            future_access_count >= future_window_access_threshold)
        if actual != expected_actual:
            raise ValueError(
                "actual_label is inconsistent with future count and threshold"
            )
        pre_access_cold = (
            past_window_access_count < prediction_context_threshold
        )
...
def _report_text(result: RunAnalysis) -> str:
    ...
    lines.extend((
        ...
        "标签 margin 为 `log2p1(future_access_count) - log2p1(K)`，绝对值"
        "不小于 0.5 视为位于明确标签一侧。",
        "",
    ...
    lines.extend((
        ...
        "## Feature 相关性诊断",
        "",
        "| 负载 | 标签相关性最强 feature | 相关系数 | feature 0/2 互相关 |",
        "|---|---|---:|---:|",
    ))
    ...
    lines.extend((
        ...
        "## 使用边界",
        "",
        "本报告解释已记录在线策略，不进行 ARF 精确回放。离线消融必须先通过"
        " baseline 概率/标签一致性门槛。",
        "",
    ))
    return "\n".join(lines)
