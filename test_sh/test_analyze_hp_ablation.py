#!/usr/bin/env python3

import csv
import json
import sys
import tempfile
import unittest
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent))

from hp_ablation_analysis import (
    ProfileDefinition,
    analyze_ablation_run,
    write_ablation_outputs,
)


class AblationAnalysisTest(unittest.TestCase):
    def test_profile_uses_the_current_feature_schema_names(self):
        profile = ProfileDefinition("drop_current_heat", (2,))
        self.assertEqual(
            profile.disabled_feature_names,
            "current_heat_log2p1",
        )

    def test_candidate_metrics_deltas_and_acceptance(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            run_root = root / "run"
            replay_root = root / "replay"
            output_root = root / "output"
            workload = "fixture_workload"
            trace_dir = run_root / workload / "trace"
            trace_dir.mkdir(parents=True)

            (run_root / workload / "phase_intervals.tsv").write_text(
                "sample_start\tsample_end\tphase_index\tphase_name\tsegment\n"
                "1970-01-01T00:00:00+00:00\t"
                "1970-01-01T00:00:10+00:00\t0\tsteady_0\tsteady\n",
                encoding="utf-8",
            )
            metadata = {
                "magic": "HPTRACE1",
                "schema_version": 2,
                "header_size": 192,
                "record_size": 200,
                "feature_count": 6,
                "osd_id": 0,
                "session_id": 1,
                "start_wall_time_ns": 1_000_000_000,
                "start_monotonic_time_ns": 100,
                "config_hash": "1",
                "git_commit": "fixture",
                "phase": workload,
                "record_count": 2,
            }
            (trace_dir / "osd.0.csv.metadata.json").write_text(
                json.dumps(metadata), encoding="utf-8"
            )
            source_fields = [
                "io_sequence", "object_key_hash", "prediction_time_ns",
                "label_completion_time_ns", "predicted_hot_probability",
                "hot_predict_threshold", "predicted_label", "actual_label",
            ]
            source_rows = [
                ["1", "10", "100", "200", "0.8", "0.5", "1", "1"],
                ["2", "20", "200", "300", "0.2", "0.5", "0", "0"],
            ]
            with (trace_dir / "osd.0.csv").open(
                "w", newline="", encoding="utf-8"
            ) as stream:
                writer = csv.writer(stream)
                writer.writerow(source_fields)
                writer.writerows(source_rows)

            profile_path = root / "profiles.tsv"
            profile_path.write_text(
                "profile\tdisabled_features\nA0\tnone\nA1\t3\n",
                encoding="utf-8",
            )
            replay_fields = [
                "io_sequence", "object_key_hash", "prediction_time_ns",
                "label_completion_time_ns", "online_hot_probability",
                "replay_hot_probability", "probability_abs_error",
                "hot_predict_threshold", "online_label", "replay_label",
                "actual_label", "cold_start_fallback",
            ]
            profile_rows = {
                "A0": [
                    ["1", "10", "100", "200", "0.8", "0.8", "0", "0.5", "1", "1", "1", "0"],
                    ["2", "20", "200", "300", "0.2", "0.2", "0", "0.5", "0", "0", "0", "0"],
                ],
                "A1": [
                    ["1", "10", "100", "200", "0.8", "0.8", "0", "0.5", "1", "1", "1", "0"],
                    ["2", "20", "200", "300", "0.2", "0.8", "0.6", "0.5", "0", "1", "0", "0"],
                ],
            }
            for profile, rows in profile_rows.items():
                destination = replay_root / profile / workload
                destination.mkdir(parents=True)
                with (destination / "osd.0.replay.tsv").open(
                    "w", newline="", encoding="utf-8"
                ) as stream:
                    writer = csv.writer(stream, delimiter="\t")
                    writer.writerow(replay_fields)
                    writer.writerows(rows)

            analysis = analyze_ablation_run(
                run_root, replay_root, profile_path, baseline_profile="A0"
            )
            baseline = analysis.global_metrics["A0"]
            candidate = analysis.global_metrics["A1"]
            self.assertEqual(baseline.accuracy, 1.0)
            self.assertEqual(candidate.accuracy, 0.5)
            self.assertEqual(candidate.accuracy_delta_pp, -50.0)
            self.assertFalse(analysis.profile_decisions["A1"].accepted)
            self.assertEqual(
                analysis.workload_metrics[("A1", workload)].predicted_hot_ratio,
                1.0,
            )

            write_ablation_outputs(analysis, output_root)
            for name in (
                "profile_summary.tsv",
                "workload_metrics.tsv",
                "phase_metrics.tsv",
                "deltas_vs_baseline.tsv",
                "REPORT.md",
            ):
                self.assertTrue((output_root / name).is_file(), name)
            report = (output_root / "REPORT.md").read_text(encoding="utf-8")
            self.assertIn("A1", report)
            self.assertIn("淘汰", report)


if __name__ == "__main__":
    unittest.main()
