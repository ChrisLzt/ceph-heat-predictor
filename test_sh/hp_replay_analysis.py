...
def _analyze_file(
    ...
    if (
        metadata.osd_id != osd_id
        or not _trace_phase_matches_workload(metadata.phase, workload)
    ):
...
def write_replay_outputs(analysis: ReplayRunAnalysis, output_root: Path) -> None:
    ...
    lines.extend([
        ...
        "## 结论",
        "",
        "Baseline replay 已达到 OSD 与 phase 两级一致性门槛，"
        "可以基于同一批 Trace 开始受控 feature 消融。"
        if analysis.gate_passed
        else "Baseline replay 未达到一致性门槛，不得据此进行 feature 消融。",
    ])
