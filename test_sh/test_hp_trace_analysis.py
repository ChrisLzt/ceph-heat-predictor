...
class TraceMetadataTest(unittest.TestCase):
    ...
    def test_rejects_legacy_trace_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaises(ValueError):
                TraceMetadata.from_json(
                    self.write_metadata(
...
class TraceAnalyzerTest(unittest.TestCase):
    ...
    def row(
        ...
        future_access_threshold: int = 100,
        past_window_access_count: int = 0,
    ) -> dict[str, str]:
        values: dict[str, object] = {
            ...
            "future_access_threshold_at_prediction":
                int(future_access_threshold),
            "past_window_access_count": past_window_access_count,
            "tracked_access_count_after_current_access": 1,
            "time_since_previous_access_ns": 0,
            "future_window_access_count": int(future_access_count),
            "future_window_access_threshold": int(future_access_threshold),
            "outcome": "evaluated",
    ...
    def test_consumes_rows_into_all_primary_aggregates(self) -> None:
        analyzer = TraceAnalyzer(feature_count=3)
        rows = (
    ...
    def test_cold_origin_confusion_measures_activation_quality(self) -> None:
        ...
        rows = (
            self.row(1, 10, 1.0, 0.8, 1, 200.0, 1,
                     future_access_threshold=50,
                     past_window_access_count=0),
            self.row(2, 20, 1.0, 0.2, 0, 200.0, 1,
                     future_access_threshold=50,
                     past_window_access_count=0),
            self.row(3, 30, 1.0, 0.8, 1, 50.0, 0,
                     future_access_threshold=100,
                     past_window_access_count=0),
            self.row(4, 40, 1.0, 0.8, 1, 200.0, 1,
                     heat_after_current_access=200.0,
                     future_access_threshold=50,
                     past_window_access_count=100),
        )
        for row in rows:
            analyzer.consume(row, self.context())
