...
void test_evaluated_sample_mapping()
{
  ...
  require(record.future_access_threshold_at_prediction == 4,
          "trace must preserve prediction-time K");
  require(record.past_window_access_count == 3,
          "trace must preserve the strict past-window count");
  require(record.tracked_access_count_after_current_access == 4,
          "trace must preserve the post-access cumulative count");
  require(record.actual_label == 1 && record.predicted_label == 1,
          "trace labels must be preserved");
  require(record.future_window_access_count == 3,
          "trace must preserve the completed future-window count");
  require(record.future_window_access_threshold == 5,
          "trace must preserve the deadline-window K");
