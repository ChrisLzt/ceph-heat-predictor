...
PIN_PATH = Path(__file__).with_name('workload_common') / 'single_v2/data/ses_source_pin.json'
OVERLAY_VERSION = 'single-v2-ses-1-cloudlab-current-alias'
...
def run(config, output, vdbench, ses_root, metadata):
    ...
    if metadata.get('profile') not in ('baseline', 'zipf099', 'file_zipf099', 'phase_zipf099', 'current'):
        raise ValueError('SES adapter requires an approved baseline or Zipf profile')
    if metadata.get('profile') == 'current':
        current = metadata.get('provenance', {}).get('current_suite', {})
        if current.get('source_profile') != 'phase_zipf099' or current.get('revision') != '2026-09-17':
            raise ValueError('Unverified current-profile alias')
