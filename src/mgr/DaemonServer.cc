...
#include "mgr/ObjectHeatPredictorCommands.h"
  ...
  std::shared_ptr<CommandContext>& cmdctx)
{
  ...
  } else if (ceph::mgr::is_object_hp_command(prefix)) {
    r = ceph::mgr::handle_object_hp_command(
      prefix, cmdctx->cmdmap, f.get(), cmdctx->odata, ss, cluster_state, daemon_state,
      [this](int32_t osd) {
        auto p = osd_cons.find(osd);
        return p != osd_cons.end() && !p->second.empty();
      },
      [this]() -> Objecter& { return py_modules.get_objecter(); });
    cmdctx->reply(r, ss);
    return true;
