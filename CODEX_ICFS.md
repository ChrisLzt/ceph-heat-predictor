...
4. vdbench 测试时 `hp_io_count` 增长。
5. `hp_labeled_io_total`、TP/FP/TN/FN、precision、recall 有变化。
6. reset 后计数归零。

如果 `hp_io_count` 不增长，优先检查当前池是否走未覆盖的新 PG 路径，或 workload 是否只产生了未纳入统计的管理类 op。
