...
**Status:** Implemented and correctness-verified on `dev`
...
Expiring due items before that increment also excludes an access exactly at or
after an old item's deadline. The right-open interval makes foreground I/O and
the background deadline worker agree regardless of which one acquires the EQ
lock first. Together these rules preserve `(t, t + 10s)`.
...
- Threshold observations are applied as soon as labels complete, independently
  of prediction completion. A late prediction may delay model training and the
  confusion matrix, but it cannot delay or reorder the object-level Otsu
  population.
