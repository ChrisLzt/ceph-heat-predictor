...
**Status:** Tasks 0-5 completed on `dev`; formal workload comparison remains.
...
At deadline, before accounting an access whose timestamp is equal to that
deadline:
...
and training independently after prediction completion. Submit the threshold
observation immediately when the label completes, before waiting for a late
prediction, so object-level Otsu progress is independent of model latency.

Add probes for:

- accesses before, exactly at, and after a deadline, proving that the exact
  deadline belongs to neither item's future window;
