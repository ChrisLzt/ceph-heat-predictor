...
预测时保存两个 corrected_count，延迟训练和 Trace 使用该时刻的值。
...
退出屏障和重复 shutdown。`bash test_sh/test_hp_osd_module.sh` 使用本地
`libceph-common` 在隔离进程中测试真实命令注册、事件过滤、实例隔离、并发控制、
Trace 轮转及 perf logger 回收，不连接现有集群。该入口也支持 `HP_SANITIZERS`。
