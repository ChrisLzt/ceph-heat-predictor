...
`short_window_access_count`、`predicted_hot_probability` 和
`predicted_label`。
...
六特征参考实现中的 `long_window_access_count_log2p1`、`heat_concentration` 和
`access_rate_change_log2p1` 已从模型输入移除。离线 mask replay 选择 C4 后，真实三维
五负载在线验证的平均 accuracy/balanced accuracy 分别提升 `0.2031/0.1307` 个百分点，
但 AI training accuracy 下降 `0.2898` 个百分点，未通过单负载最大下降 `0.2` 个百分点
的 gate。因此不再保留 C4 的第四维 crossing feature。结果见
[C4 在线验证](../../ceph-test/new_workload/hp_runs/reports/20260717_035300_feature_c4_online/REPORT.md)。
...
- 预测：固定阈值 `0.50`，不维护动态校准状态。训练直接使用单位样本权重，冷热标签
  均为 `1.0`，不存在类别专用权重。
- 容量：`HP_FUTURE_LABEL_WINDOW_NS=10s`、兼容实验参数
  `HP_SHORT_ACCESS_WINDOW_NS=5s`、
...
Otsu score 原点和访问距离全部使用 `steady_clock`；当前访问速率 feature 关闭，不会
实例化5秒/10秒窗口事件。I/O 序号只保留用于样本顺序和统计。Otsu 维护 object 当前
...
- `summary.trace`：启用 Trace 的 OSD 数、待写队列、已写、丢弃和写错误计数。
...
`dev` 支持默认关闭的已完成评估 I/O Trace；`main` 不包含该实验功能：

```bash
ceph osd hp trace start <阶段名> /var/log/ceph
ceph osd hp trace stop
python3 test_sh/convert_hp_trace.py <trace.bin> -o <trace.csv>
```

每个 OSD 写独立的定长二进制 session。前台通过原子占位写容量为 `65536` 的有界
MPSC ring，只有 ring 真正满时才丢 Trace；后台单消费者线程批量落盘。重复 start 和
`reset/enable/disable` 会排空旧队列并轮换 session。文件头记录 schema、OSD、phase、
Git 版本和配置哈希；正常评估与 EQ 容量丢弃、预测错误分开编码。详细格式见
[Trace 设计](TRACE_DATASET.md)。
