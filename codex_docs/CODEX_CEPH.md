# Ceph Object Heat Predictor V1

本文记录生产分支 `main` 的 Heat Predictor V1 实现。当前发布版本为 `v1.0`
（commit `52404b0ff2`）；参数以 `src/heatpredictor/hp_config.h` 为准。

V1 对每条 I/O 在预测时保存 feature 和 Otsu 热度阈值，10秒后把同一 object 在
...
ARF 其余参数为 grace `100`、lambda `4`、delta `0.001`、tau `0.05`、
max-share `0.99`、min-branch `0.01`；warning/drift ADWIN delta 分别为
`0.010/0.001`。快速树数量固定为0。
...
PerfCounters。MGR 读取 OSD 周期上报的 PerfCounters，可能短暂滞后；测试结束等待队列
排空时应优先检查每个 OSD 的实时接口。完整流程见
...
- `osds`：up/reporting/enabled/disabled/missing OSD
- `samples`：I/O、labeled、pending、awaiting、drop
- `heat_state`：heat/LRU/protected/peak/eviction、Otsu 投票、阈值和状态
- `confusion_matrix`：TP/FP/TN/FN
- `actual_behavior`：热/冷 I/O 样本的未来访问次数均值和 OSD 分位数加权平均
- `prediction`：accuracy、balanced accuracy、precision、recall、预测/实际热比例
...
2026-07-24 五负载单轮验证的加权 Accuracy/Balanced Accuracy 为
`87.76%/88.27%`，仅作为该实现的历史回归参考，不是固定性能承诺。报告位于：

```text
/home/chris/ceph-test/new_workload/hp_runs/reports/
20260724_004418_v1_2_prediction_threshold_snapshot_all5/REPORT.md
```
