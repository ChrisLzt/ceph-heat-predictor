...
- `hp_algorithm_probe` 验证文件头、正常/异常记录、后台排空和 session 轮换。
...
回放工具只接受 schema v2 中已完成评估的记录。每个 OSD 独立按
...
g++ -std=c++17 -O2 -pthread \
  ...
  test_sh/hp_trace_replay.cc -o /tmp/hp_trace_replay
...
回放 MapReduce `osd.0`；任一指标失败就停止 schema v2 实验，不运行其他 OSD，也不做
feature 消融。

Schema v2 没有保存预测快照 generation/version，因此只能解释已记录策略，不能保证模型
快照发布时序完全复现。若 baseline parity 无法通过，应先给 Trace 增加快照代次，而不是放宽
一致性标准。

2026-07-17 baseline 结果：10 个 OSD、1,468,315 条记录和 46 个 workload phase 全部通过
上述 gate，原始 Trace 校验和保持不变。该历史结果来自 schema v1；当前 schema v2 删除了
未来新增热度字段，旧文件不能直接按新 schema 回放。历史结果位于
`hp_runs/reports/20260717_002308_dev_trace_all5/offline_replay_all5/`。

## Feature 消融结果

2026-07-17 的六维 mask replay 选择 C4：保留热度阈值 margin、上次访问间隔编码和当前
热度，屏蔽长期访问次数、平均单次访问热度和短/长访问速率变化。C4 在1,468,315条固定
样本上使 Accuracy/Balanced Accuracy 分别增加 `0.1118/0.0071` 个百分点，且通过离线
负载和 phase gate。

真实三维 C4 的五负载单轮在线验证使平均 Accuracy/Balanced Accuracy 分别增加
`0.2031/0.1307` 个百分点，但 AI training Accuracy 下降 `0.2898` 个百分点，超过
`0.2` 个百分点 gate。阶段统计显示 C4 稳态更好、冷启动和热点切换更差。

mask replay 仍构造六维 ARF/scaler，只把禁用维度置零；真实三维模型会改变树分裂候选和
固定随机数消费顺序。若继续定位差异，应增加把旧 schema v1 六特征记录投影为真实三维
模型输入的 replay profile，不能用 mask replay 代替在线验收。

报告：

- `hp_runs/reports/20260717_030527_feature_ablation_replay/REPORT.md`
- `hp_runs/reports/20260717_035300_feature_c4_online/REPORT.md`
