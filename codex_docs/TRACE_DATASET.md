...
默认 direct-ARF 回放关闭ADWIN，使用当前构建的快照样本门槛、最长间隔和3000成熟快照样本预热保护。
`baseline`/`conservative` 漂移实验必须显式选择，不代表当前线上默认策略。
预热只改变预测输出，不跳过训练；保护是否结束由已发布模型的训练计数决定。
开发端 `HpReplayOptions::warmup_trained_samples=0` 可显式关闭保护以隔离空模型行为，
不代表当前线上默认值。持久性/残差实验模式仍有各自显式策略。
Trace 配置哈希包含预热门槛、多数类叶策略和发布频率，旧配置Trace不会默认当作新配置。
回放仍不模拟真实训练积压、丢弃和跨线程可见性，不能保证逐条复现线上输出。

## C4 特征兼容性

二进制 schema 4 的八个 feature 槽位和184字节记录不变；当前 feature_count 为7。
feature 0–4 保持原定义，5/6 是归一化倍率上限4的30s/60s慢历史裕量。
特征策略版本升为8；配置哈希包含两个时间常数及倍率上限。
转换器可继续读取旧五维 Trace，但当前回放器拒绝特征数不匹配的旧文件；
历史对照须使用对应报告中冻结的旧源码，不能把空槽补零当作 C4。

## 2026-09-23 完整对象身份

新线上入口使用完整对象身份登记。`object_key_hash`字段名及二进制布局保持不变，但值为
进程内不复用的内部ID；只在同一进程及对应OSD数据中关联，不能跨进程/OSD当作相同对象。
热状态被淘汰后同一对象再次访问会登记新ID，以匹配历史重置。Trace不含完整身份反查表。
配置hash加入身份策略版本1。旧整数探针接口仍保留历史hash算法，但不得与完整身份模式
在同一EQ实例生命周期混用。回放可将记录中的ID当作不透明键，旧/新配置不得当作等价。
